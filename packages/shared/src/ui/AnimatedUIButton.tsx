...
export function getButtonSizes(size: AnimatedUISize = "md"): ButtonSizeStyles {
  return getAnimatedButtonSizeStyles(size);
}

function getDefaultVariantColor(variant: ButtonVariant, color?: StaffColorName): StaffColorName {
  if (color) {
    return color;
  }
...
function getUIButtonVariantClasses(variant: ButtonVariant, color?: StaffColorName): VariantClasses {
  ...
  switch (variant) {
    ...
    case "outline":
      return {
        ...
        text: cn(defaultColors.textColor, prefixStateClasses("group-hover", hoverColors.textColor)),
        iconColor: resolvedColor,
        iconState: "default",
      };
...
type AnimatedUIButtonAsButtonProps = AnimatedUIButtonBaseProps &
  Omit<ButtonHTMLAttributes<HTMLButtonElement>, "children" | MotionUnsafeButtonProps> & {
    href?: undefined;
  };

type AnimatedUIButtonAsLinkProps = AnimatedUIButtonBaseProps &
  Omit<AnchorHTMLAttributes<HTMLAnchorElement>, "children" | MotionUnsafeAnchorProps> & {
    href: string;
  };

export type AnimatedUIButtonProps = AnimatedUIButtonAsButtonProps | AnimatedUIButtonAsLinkProps;
...
export function AnimatedUIButton({
  ...
  if (isLink) {
    const { href, target, rel, onClick, ...anchorProps } = props as AnimatedUIButtonAsLinkProps;
    ...
    return (
      <motion.a
        ...
        className={cn(sharedClassName, loading && "pointer-events-none opacity-65")}
  ...
  const { type = "button", form, ...buttonProps } = props as AnimatedUIButtonAsButtonProps;
