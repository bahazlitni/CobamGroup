-- Seed the uploaded "Cadre en fer" images as variants in the Cadre en fer family.
...
CREATE TEMP TABLE "_cadre_attribute_definitions" (
...
INSERT INTO "_cadre_attribute_definitions" (
...
VALUES
  ...
  ('fer_diameter_mm', 'Diametre fer', 'mm', 'NUMBER'::"ProductTypeAttributeInputType", ARRAY[]::TEXT[], true, 10);
...
SELECT
  ...
  CURRENT_TIMESTAMP,
  CURRENT_TIMESTAMP
FROM "_cadre_attribute_definitions"
...
SELECT
  ...
  CURRENT_TIMESTAMP,
  CURRENT_TIMESTAMP
FROM "_cadre_attribute_definitions" "seed"
...
CREATE TEMP TABLE "_cadre_expected_media" (
  "media_id" BIGINT PRIMARY KEY,
  "expected_filename" TEXT NOT NULL,
  "kind" "MediaKind" NOT NULL
);

INSERT INTO "_cadre_expected_media" ("media_id", "expected_filename", "kind")
VALUES
  (101, 'CADRE 1010 EN FER DE 05.png', 'IMAGE'::"MediaKind"),
  (102, 'CADRE 1015 EN FER DE 05.png', 'IMAGE'::"MediaKind"),
  (103, 'CADRE 1515 EN FER DE 6.png', 'IMAGE'::"MediaKind"),
  (104, 'CADRE 1530 EN FER DE 05.png', 'IMAGE'::"MediaKind"),
  (105, 'CADRE 1540 EN FER DE 05.png', 'IMAGE'::"MediaKind"),
  (106, 'CADRE 1730 EN FER DE 05.png', 'IMAGE'::"MediaKind"),
  (107, 'CADRE 2035 EN FER DE 05.png', 'IMAGE'::"MediaKind"),
  (108, 'CADRE 2525 EN FER DE 05.png', 'IMAGE'::"MediaKind"),
  (109, 'CADRE 2530 EN FER DE 05.png', 'IMAGE'::"MediaKind");
...
BEGIN
  ...
  FROM "_cadre_expected_media" "expected"
  ...
  IF missing_media_count > 0 THEN
    RAISE EXCEPTION 'Cannot seed Cadre en fer media: % expected media row(s) are missing or mismatched.', missing_media_count;
  END IF;
END $$;

CREATE TEMP TABLE "_cadre_en_fer_products" (
...
INSERT INTO "_cadre_en_fer_products" (
  "sku", "slug", "name", "display_name", "price_ttc", "stock_available",
  "dimensions_text", "fer_diameter_mm", "sort_order", "main_media_id"
)
VALUES
  ('00207768', 'cadre-10-10-en-fer-de-05-00207768', 'CADRE 10/10 EN FER DE 05', 'Cadre 10/10 en fer de 05', 0.360, 30.000, '10/10', '5', 0, 101),
  ('00004005', 'cadre-10-15-en-fer-de-05-00004005', 'CADRE 10/15 EN FER DE 05', 'Cadre 10/15 en fer de 05', 0.420, 1246.000, '10/15', '5', 2, 102),
  ('DIV01CADR151506', 'cadre-15-15-en-fer-de-06-div01cadr151506', 'CADRE 15/15 EN FER DE 06', 'Cadre 15/15 en fer de 06', NULL, 0.000, '15/15', '6', 23, 103),
  ('00184984', 'cadre-15-30-en-fer-de-05-00184984', 'CADRE 15/30 EN FER DE 05', 'Cadre 15/30 en fer de 05', 0.730, 307.000, '15/30', '5', 24, 104),
  ('DIV01CADR40', 'cadre-15-40-en-fer-de-05-div01cadr40', 'CADRE 15/40 EN FER DE 05', 'Cadre 15/40 en fer de 05', 0.910, 200.000, '15/40', '5', 27, 105),
  ('00221160', 'cadre-17-30-en-fer-de-05-00221160', 'CADRE 17/30 EN FER DE 05', 'Cadre 17/30 en fer de 05', 0.770, 40.000, '17/30', '5', 38, 106),
  ('00237574', 'cadre-20-35-en-fer-de-05-00237574', 'CADRE 20/35 EN FER DE 05', 'Cadre 20/35 en fer de 05', 0.950, 190.000, '20/35', '5', 45, 107),
  ('00219853', 'cadre-25-25-en-fer-de-05-00219853', 'CADRE 25/25 EN FER DE 05', 'Cadre 25/25 en fer de 05', 0.810, 350.000, '25/25', '5', 53, 108),
  ('00225762', 'cadre-25-30-en-fer-de-05-00225762', 'CADRE 25/30 EN FER DE 05', 'Cadre 25/30 en fer de 05', 0.950, 10.000, '25/30', '5', 54, 109);
...
SELECT
  ...
  jsonb_build_object(
    ...
    'content', jsonb_build_array(
      ...
      jsonb_build_object(
        'type', 'paragraph',
        'content', jsonb_build_array(jsonb_build_object('type', 'text', 'text', 'Ce cadre en fer est destine aux armatures beton et aux travaux courants de construction. Sa forme preparee facilite la mise en oeuvre sur chantier et le choix de la variante selon la dimension attendue.'))
      )
    )
  ),
  left("seed"."display_name" || ' | COBAM GROUP', 60),
  left("seed"."display_name" || ' : cadre en fer pour armatures beton chez COBAM GROUP en Tunisie.', 160),
  ...
  CURRENT_TIMESTAMP,
  CURRENT_TIMESTAMP
FROM "_cadre_en_fer_products" "seed"
...
USING "products" "product", "_cadre_en_fer_products" "seed"
...
CREATE TEMP TABLE "_cadre_en_fer_media" (
  "sku" TEXT PRIMARY KEY,
  "media_id" BIGINT NOT NULL,
  "alt_text" TEXT NOT NULL
);

INSERT INTO "_cadre_en_fer_media" ("sku", "media_id", "alt_text")
SELECT
  "seed"."sku",
  "seed"."main_media_id",
  "seed"."display_name" || ' - vue produit'
FROM "_cadre_en_fer_products" "seed";
...
SELECT
  ...
  CURRENT_TIMESTAMP,
  CURRENT_TIMESTAMP
FROM "_cadre_en_fer_media" "media_link"
JOIN "products" "product"
  ON "product"."sku" = "media_link"."sku"
JOIN "_cadre_expected_media" "expected"
...
DROP TABLE "_cadre_en_fer_media";
DROP TABLE "_cadre_en_fer_products";
DROP TABLE "_cadre_expected_media";
DROP TABLE "_cadre_attribute_definitions";
