"use client";

import { useCallback, useEffect, useMemo, useRef, useState } from "react";
import { usePersistentPageSize } from "@/lib/client/use-persistent-page-size";
import {
  listProductPacksClient,
  ProductPacksClientError,
} from "../client";
import type {
  ProductPackListItemDto,
  ProductPackPageSize,
} from "../types";

const PAGE_SIZE_OPTIONS: ProductPackPageSize[] = [10, 20, 50];

export function useProductPacksList(initialPageSize: ProductPackPageSize = 20) {
  const [items, setItems] = useState<ProductPackListItemDto[]>([]);
  const [total, setTotal] = useState(0);
  const [page, setPage] = useState(1);
  const [pageSize, setPageSize] = usePersistentPageSize(
    "staff:product-packs:list:page-size",
    initialPageSize,
    PAGE_SIZE_OPTIONS,
  );
  const [search, setSearch] = useState("");
  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const pageRef = useRef(page);
  const pageSizeRef = useRef(pageSize);
  const searchRef = useRef(search);

  useEffect(() => {
    pageRef.current = page;
  }, [page]);

  useEffect(() => {
    pageSizeRef.current = pageSize;
  }, [pageSize]);

  useEffect(() => {
    searchRef.current = search;
  }, [search]);

  const totalPages = useMemo(
    () => Math.max(1, Math.ceil(total / pageSize)),
    [pageSize, total],
  );

  const fetchProductPacks = useCallback(
    async (override?: {
      page?: number;
      pageSize?: ProductPackPageSize;
      search?: string;
    }) => {
      const nextPage = override?.page ?? pageRef.current;
      const nextPageSize = override?.pageSize ?? pageSizeRef.current;
      const nextSearch = override?.search ?? searchRef.current;

      setIsLoading(true);
      setError(null);

      try {
        const result = await listProductPacksClient({
          page: nextPage,
          pageSize: nextPageSize,
          q: nextSearch,
        });

        setItems(result.items);
        setTotal(result.total);
        setPage(result.page);
        setPageSize(result.pageSize as ProductPackPageSize);
      } catch (err: unknown) {
        const message =
          err instanceof ProductPacksClientError
            ? err.message
            : err instanceof Error
              ? err.message
              : "Erreur inconnue";
        setError(message);
      } finally {
        setIsLoading(false);
      }
    },
    [setPageSize],
  );

  useEffect(() => {
    void fetchProductPacks({ page: 1, pageSize });
  }, [fetchProductPacks, pageSize]);

  const submitFilters = useCallback(async () => {
    await fetchProductPacks({
      page: 1,
      pageSize,
      search,
    });
  }, [fetchProductPacks, pageSize, search]);

  const updatePageSize = useCallback(
    async (value: ProductPackPageSize) => {
      const safeValue = PAGE_SIZE_OPTIONS.includes(value) ? value : 20;
      setPageSize(safeValue);
      await fetchProductPacks({ page: 1, pageSize: safeValue });
    },
    [fetchProductPacks, setPageSize],
  );

  const goPrev = useCallback(async () => {
    if (page <= 1) return;
    await fetchProductPacks({ page: page - 1 });
  }, [fetchProductPacks, page]);

  const goNext = useCallback(async () => {
    if (page >= totalPages) return;
    await fetchProductPacks({ page: page + 1 });
  }, [fetchProductPacks, page, totalPages]);

  return {
    items,
    total,
    page,
    pageSize,
    search,
    isLoading,
    error,
    totalPages,
    canPrev: page > 1,
    canNext: page < totalPages,
    setSearch,
    fetchProductPacks,
    submitFilters,
    updatePageSize,
    goPrev,
    goNext,
  };
}
