"use client";

import { useCallback, useEffect, useMemo, useRef, useState } from "react";
import {
  listProductCategoriesClient,
  ProductCategoriesClientError,
} from "../client";
import type {
  ProductCategoryListItemDto,
  ProductCategoryPageSize,
} from "../types";
import { usePersistentPageSize } from "@/lib/client/use-persistent-page-size";

const PAGE_SIZE_OPTIONS: ProductCategoryPageSize[] = [10, 20, 50];

export function useProductCategoriesList(
  initialPageSize: ProductCategoryPageSize = 20,
) {
  const [items, setItems] = useState<ProductCategoryListItemDto[]>([]);
  const [total, setTotal] = useState(0);
  const [page, setPage] = useState(1);
  const [pageSize, setPageSize] = usePersistentPageSize(
    "staff:product-categories:list:page-size",
    initialPageSize,
    PAGE_SIZE_OPTIONS,
  );
  const [search, setSearch] = useState("");
  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const pageRef = useRef(page);
  const pageSizeRef = useRef(pageSize);
  const searchRef = useRef(search);

  useEffect(() => {
    pageRef.current = page;
  }, [page]);

  useEffect(() => {
    pageSizeRef.current = pageSize;
  }, [pageSize]);

  useEffect(() => {
    searchRef.current = search;
  }, [search]);

  const totalPages = useMemo(
    () => Math.max(1, Math.ceil(total / pageSize)),
    [pageSize, total],
  );

  const fetchProductCategories = useCallback(
    async (options?: {
      page?: number;
      pageSize?: ProductCategoryPageSize;
      search?: string;
    }) => {
      const nextPage = options?.page ?? pageRef.current;
      const nextPageSize = options?.pageSize ?? pageSizeRef.current;
      const nextSearch = options?.search ?? searchRef.current;

      setIsLoading(true);
      setError(null);

      try {
        const result = await listProductCategoriesClient({
          page: nextPage,
          pageSize: nextPageSize,
          q: nextSearch,
        });

        setItems(result.items);
        setTotal(result.total);
        setPage(result.page);
        setPageSize(result.pageSize as ProductCategoryPageSize);
      } catch (err: unknown) {
        const message =
          err instanceof ProductCategoriesClientError
            ? err.message
            : err instanceof Error
              ? err.message
              : "Erreur inconnue";
        setError(message);
      } finally {
        setIsLoading(false);
      }
    },
    [setPageSize],
  );

  useEffect(() => {
    void fetchProductCategories({ page: 1, pageSize });
  }, [fetchProductCategories, pageSize]);

  const submitSearch = useCallback(async () => {
    await fetchProductCategories({ page: 1, pageSize, search });
  }, [fetchProductCategories, pageSize, search]);

  const updatePageSize = useCallback(
    async (value: ProductCategoryPageSize) => {
      const safeValue = PAGE_SIZE_OPTIONS.includes(value) ? value : 20;
      setPageSize(safeValue);
      await fetchProductCategories({ page: 1, pageSize: safeValue });
    },
    [fetchProductCategories, setPageSize],
  );

  const goPrev = useCallback(async () => {
    if (page <= 1) return;
    await fetchProductCategories({ page: page - 1 });
  }, [fetchProductCategories, page]);

  const goNext = useCallback(async () => {
    if (page >= totalPages) return;
    await fetchProductCategories({ page: page + 1 });
  }, [fetchProductCategories, page, totalPages]);

  return {
    items,
    total,
    page,
    pageSize,
    search,
    isLoading,
    error,
    totalPages,
    canPrev: page > 1,
    canNext: page < totalPages,
    setSearch,
    fetchProductCategories,
    submitSearch,
    updatePageSize,
    goPrev,
    goNext,
  };
}
