...
function dedupeEntries(entries: MetadataRoute.Sitemap): MetadataRoute.Sitemap {
  ...
  for (const entry of entries) {
    ...
    const currentLastModified = current.lastModified ? new Date(current.lastModified) : null;
    const nextLastModified = entry.lastModified ? new Date(entry.lastModified) : null;

    if (nextLastModified && (!currentLastModified || nextLastModified > currentLastModified)) {
      seen.set(entry.url, entry);
    }
  }
...
function publicSubcategoryWhere(
  ...
  return hasVisibilityColumns ? { isActive: true, visibleVitrine: true } : { isActive: true };
}
...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const visibleSubcategoryLinkWhere = publicProductSubcategoryLinkWhere(hasVisibilityColumns);

  const [categories, subcategories, products, families, articles] = await Promise.all([
    ...
    prisma.product.findMany({
      where: {
        OR: [
          { kind: { in: [ProductKind.STANDARD, ProductKind.SINGLE] } },
          {
  ...
  const entries: MetadataRoute.Sitemap = [
    ...
    ...subcategories.map((subcategory) =>
      mapUrl(`/produits/${subcategory.category.slug}/${subcategory.slug}`, subcategory.updatedAt, {
        changeFrequency: "weekly",
        priority: 0.75,
      }),
    ),
    ...
    ...families.flatMap((family) => {
      const link = family.members.flatMap((member) => member.product.subcategories).at(0);
