...
export default function RailCarousel({
  ...
  const commitScroll = (x: number) => {
    const el = viewportEl.current;
    if (!el) return;
    el.scrollLeft = shouldLoop ? wrapPosition(x) : Math.max(0, Math.min(getMaxScroll(), x));
  };
    ...
    >
      {showAnyButton && (
        <>
          <AnimatedUIButton
            ...
            className={cn(
              "absolute top-1/2 left-0 z-10 h-10 min-h-0 w-10 -translate-y-1/2 rounded-full",
          ...
          <AnimatedUIButton
            ...
            className={cn(
              "absolute top-1/2 right-0 z-10 h-10 min-h-0 w-10 -translate-y-1/2 rounded-full",
              "border-slate-200 bg-white/95 px-0 py-0 shadow-sm transition-opacity duration-150",
              showButtons === "always" ? "opacity-100" : "opacity-0 group-hover/rail:opacity-100",
      ...
      <div
        ...
        className={cn(
          ...
          allowDrag && hasOverflow && "cursor-grab touch-pan-y select-none",
          viewportClassName,
        )}
